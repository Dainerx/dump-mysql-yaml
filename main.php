...
function read_args()
{
    ...
    global $argc, $argv, $DBHOST, $DBUSER, $DBPASS, $DBNAME, $FILE, $TABLES, $DBQUERY, $QUERYNAME, $CONVERT_NAMES;
 ...
 */
function sql_to_yaml($link, $sql, $table, $file = null)
{
    ...
    if ($file != null) {
        $ymlFile = fopen($file, "a+") or die("Unable to open file!");
        if ($result) {
            // Output the table name
            fwrite($ymlFile, ($CONVERT_NAMES === true) ? convert_name_to_class($table) . ":\n" : "{$table}:\n");
            ...
            while ($row = mysqli_fetch_assoc($result)) {
                ...
                foreach ($row as $key => $value) {
                    ...
                    fwrite($ymlFile, "    {$key}: {$literalFlag}{$value}\n");
                }
            }
        }
        fclose($ymlFile);
...
if (read_args()) {
    ...
    if (mysqli_select_db($link, $DBNAME)) {
        ...
        // Query Mode
        if (!empty($DBQUERY)) {
            sql_to_yaml($link, $DBQUERY, $QUERYNAME, $FILE);
        } else {
            ...
            foreach ($TABLES as $tbl) {
                sql_to_yaml($link, "SELECT * FROM {$tbl}", $tbl, $FILE);
                if ($FILE == null)
                    echo "\n";
                else
                    writeToFile("\n");
